...
const FooterSection = ({ title, children }: { title: string; children: React.ReactNode }) => (
  <div>
    <h3 className="mb-4 font-heading text-lg font-semibold text-foreground">{title}</h3>
    {children}
  </div>
);
    ...
    <footer className="border-t border-border bg-accent text-accent-foreground">
      <div className="mx-auto max-w-7xl px-4 py-12 md:px-6">
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {/* School Info */}
          <div>
            <h3 className="mb-4 font-heading text-xl font-semibold">{config.schoolName}</h3>
          ...
          <FooterSection title={t('footer.followUs')}>
            <div className="flex gap-3">
              <a
                ...
                className="rounded-full bg-muted p-2.5 text-muted-foreground transition-colors hover:bg-primary hover:text-white"
                aria-label="Facebook"
              >
